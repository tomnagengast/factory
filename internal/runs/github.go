...
// PullRequestDiscoverer finds any pull request whose head starts with the
// canonical task branch prefix. Mechanical completion uses it to reject a
// success that lacks a manager-validated checkpoint.
...
func (c *GitHubCLI) Snapshot(ctx context.Context, checkpoint ReadyCheckpoint) (PullRequestSnapshot, error) {
	...
	snapshot := PullRequestSnapshot{
		Number: checkpoint.PullRequest, State: value.State, IsDraft: value.IsDraft,
		BaseBranch: value.BaseRefName, HeadBranch: value.HeadRefName, HeadOID: value.HeadRefOID,
		SafeguardRegression: pullRequestSafeguardRegression(value.Review, value.Checks),
...
func (c *GitHubCLI) MatchingIssuePullRequests(ctx context.Context, repository, branchPrefix string) ([]PullRequestSnapshot, error) {
	...
	for _, value := range values {
		...
		snapshot := PullRequestSnapshot{
			Number: value.Number, State: value.State, IsDraft: value.IsDraft,
			BaseBranch: value.BaseRefName, HeadBranch: value.HeadRefName, HeadOID: value.HeadRefOID, UpdatedAt: value.UpdatedAt,
		}
		if value.MergeCommit != nil {
			snapshot.MergeCommitOID = value.MergeCommit.OID
		}
