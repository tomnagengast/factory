...
Finish only after the implementation satisfies the issue and reviewed plan, all relevant verification passes, a human merges the exact verified pull-request head, merged checks and review safeguards remain clear, deployment succeeds from updated clean main or the plan proves none applies, remote and local issue branches are cleaned up, and Linear contains the final evidence with no unanswered feedback.
...
2. Resolve work only through the Linear project's allowlisted repository metadata. In a managed Factory run, use `FACTORY_REPO_PATH` as the mutable repository and deployment source after confirming its origin matches the project metadata.
...
Save research at `plans/planning/<branch>/research.md`, commit it, push it, and maintain one draft pull request. Refresh Linear labels immediately before the research gate. A current `Yolo` label permits immediate progression after the normal artifact and gate comment are published. Otherwise require explicit human approval through an affirmative reaction or contextual reply. Address requested revisions in the exact thread and do not plan while the gate is waiting.
...
Push the implementation, update the draft pull-request body with problem, decisions, risks, non-goals, exact verification evidence, approved-plan path, and exact locally verified head, then mark it ready for review. Ensure Linear links the pull request, move the issue to the unambiguous review state, and publish one implementation summary with the verified head.
...
On a post-merge segment, reconstruct the repository, pull request, base, head, and verified head from the retained checkpoint and durable pull-request or Linear evidence. Fresh-read GitHub and Linear. Continue only when GitHub authoritatively reports a merge commit and the reported merge contains the exact checkpointed head as an ancestor. A squash or rebase replay without that ancestry is not the verified head.

Repeat the final check, review, comment, thread, and Linear safeguards. Resolve exactly one main Worktrunk checkout and require it to be the managed repository. Refuse to stash, reset, overwrite, or deploy around a dirty or divergent main checkout.
