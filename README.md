...
Every run receives machine-wide repository authority. The routed repository stays the primary deployment source and completion anchor, while `FACTORY_REPOSITORIES` exposes the full admitted catalog so a plan may include coordinated changes in additional repositories - one branch, pull request, human merge, and exact verified-head proof per repository. A run never needs a separately routed coordination issue to change another admitted repository.
...
- `/agents` is the authenticated run dashboard with task context, lifecycle phase, ready-checkpoint PR and verified head, authoritative refresh timing, resume counts, deployment receipt identity, and terminal rejection evidence.
- `/agents/<task-identifier>/<started-unix-ms>/run?source=factory|linear` is the canonical authenticated, read-only observer for one started native or Linear run. Retained queryless references remain readable when the identifier and start millisecond select only one task source.
...
Startup reconciles the compiled reserved workflows: a missing entry is appended and an entry from an older compiled revision is upgraded, while an operator-published newer revision is preserved. A same-revision divergence stops startup instead of being silently overwritten. Native start rechecks the enabled project, succeeded project setup, repository catalog, and the published reserved workflow before it snapshots the route and admits work; continuations pin the currently published reserved workflow so admitted tasks keep working across workflow upgrades. The committed default enables no project. After ENG-46 deployment, rollout remains limited to the admitted Factory project and one separately scoped native canary. The canary must independently cross its research and plan gates and complete its own checkpointed pull request, human merge, deployment, and cleanup. Network-provider workflow work and broader project enablement remain separate follow-ups.
...
- `FACTORY_REPO_PATH`
- `FACTORY_REPOSITORIES`, the admitted repository catalog as JSON (repository, path, base branch).
...
The home and active agent views poll their APIs every two seconds. Each started run links to `https://factory.nags.cloud/agents/<task-identifier>/<started-unix-ms>/run?source=factory|linear`; pending runs remain non-link rows. Native and Linear Runs use the same observer. API, authorization, decoding, and network failures leave the loading state and render an observer error. Every successful observer response includes its observation time, current retry attempt, tmux windows, commands, and recent pane output. Agent events appear in provider chronology as action-first rows: narrative stays readable in sequence, while command, search, file, and tool work names the useful target instead of leading with provider status or type. Expanding a row shows the available normalized command or input, output, and error evidence, followed by the complete redacted raw event for diagnosis. Completed rows stay visually quiet; running and failed rows retain explicit text as well as color. Stable provider IDs keep an expanded row open while a started command completes or a Claude tool result updates its original action. Unknown structured events retain a directly inspectable raw fallback. When tmux exits, the observer reconstructs the complete principal-attempt and child-agent histories from their retained JSONL event files without the live pane limit. Terminal views stop polling after loading this immutable history. Plain terminal output remains available when a pane does not contain structured events. A live session that cannot be observed is reported as an observer error instead of an empty session. The view never accepts terminal input. Use the attach command shown on the page when interactive local control is required.
