                    ...
                    <div class="task-detail-grid">
                      <section class="task-main-column">
                        <form class="task-edit-form task-panel" onSubmit={(event) => void saveTask(event)}>
                          ...
                          <label><span>Title</span><input required maxlength="200" value={title()} onInput={(event) => setTitle(event.currentTarget.value)} /></label>
                          <label><span>Description</span><textarea maxlength="65536" rows="10" value={description()} onInput={(event) => setDescription(event.currentTarget.value)} /></label>
                          <label><span>Approval mode</span><select value={approval()} onChange={(event) => setApproval(event.currentTarget.value as "gated" | "automatic")}><option value="gated">Human gates</option><option value="automatic">Automatic progression</option></select></label>
                          <button class="secondary-button" type="submit" disabled={busy() !== ""}>Save task</button>
                        </form>
                        ...
                        <section class="task-panel" aria-labelledby="conversation-title">
                          ...
                          <Show when={snapshot().messages.messages.length > 0} fallback={<div class="empty-state task-empty"><strong>No messages yet.</strong><span>Leave context for the next human or agent continuation.</span></div>}>
                            <ol class="task-messages"><For each={snapshot().messages.messages}>{(item) => <li classList={{ reply: Boolean(item.parentId) }}><div><strong>{taskActorLabel(item.author)}</strong><time datetime={item.createdAt}>{formatTime(item.createdAt)}</time></div><p>{item.body}</p><button class="text-button" type="button" onClick={() => { setReplyTo(item.id); setMessage(""); }}>Reply</button></li>}</For></ol>
                          </Show>
                          <form class="task-inline-form" onSubmit={async (event) => { event.preventDefault(); const ok = await mutate(replyTo() ? "Posting reply" : "Posting message", "/messages", { expectedRevision: task().revision, parentId: replyTo(), body: message() }); if (ok) { setMessage(""); setReplyTo(""); } }}>
                      ...
                      <aside class="task-side-column">
                        ...
                        <section class="task-panel" aria-labelledby="gates-title">
                          ...
                          <ol class="task-gates"><For each={snapshot().gates}>{(gate) => <li><div><strong>{runStateLabel(gate.kind)}</strong><span class={`gate-status ${gate.status}`}>{runStateLabel(gate.status)}</span></div><p>{gate.mode === "gated" ? "Human decision required" : "Automatic gate"}</p><Show when={gate.artifactUrl}><a class="text-link" href={gate.artifactUrl} target="_blank" rel="noreferrer">Open artifact</a></Show><Show when={gate.status === "open" && gate.mode === "gated"}><label><span>Decision note</span><textarea rows="2" value={decisionReason()} onInput={(event) => setDecisionReason(event.currentTarget.value)} /></label><div class="task-form-actions"><button class="primary-button" type="button" disabled={busy() !== ""} onClick={() => void mutate("Approving gate", `/gates/${gate.id}/decision`, { expectedRevision: task().revision, action: "approve", reason: decisionReason() })}>Approve</button><button class="secondary-button" type="button" disabled={busy() !== ""} onClick={() => void mutate("Requesting revision", `/gates/${gate.id}/decision`, { expectedRevision: task().revision, action: "revise", reason: decisionReason() })}>Request revision</button></div></Show></li>}</For></ol>
                          <form class="task-inline-form" onSubmit={async (event) => { event.preventDefault(); const ok = await mutate("Opening gate", "/gates", { expectedRevision: task().revision, kind: gateKind(), mode: gateMode(), artifactUrl: artifactURL() }); if (ok) setArtifactURL(""); }}><label><span>Gate kind</span><input required maxlength="80" value={gateKind()} onInput={(event) => setGateKind(event.currentTarget.value)} /></label><label><span>Mode</span><select value={gateMode()} onChange={(event) => setGateMode(event.currentTarget.value as "gated" | "automatic")}><option value="gated">Human gate</option><option value="automatic">Automatic</option></select></label><label><span>Artifact URL</span><input type="url" pattern="https://.*" value={artifactURL()} onInput={(event) => setArtifactURL(event.currentTarget.value)} /></label><button class="secondary-button" type="submit" disabled={busy() !== ""}>Open gate</button></form>
                        </section>
                      </aside>
                    </div>
